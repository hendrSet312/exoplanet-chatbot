...
class modelling:
    def __init__(self, data_parameter : dict):
        ...
        if 'P_NAME' in data_parameter : 
            self.__data = self.__data.drop(columns=['P_NAME'])
    ...
    def shap_explainer(self, prediction_data, result : int):
        ...
        link = f"media/image/{self.__name}.png"
        plt.savefig(link, bbox_inches="tight", dpi=300)

        sample_shap_values = shap_values[0].values[:,result].round(2)
